...
// Wii U mappings might differ from Wii.
// These are placeholders based on Wii U port.
// Ideally should use SDL_GameController or check specific Wii mappings.
#define JOY_A     0
#define JOY_B     1
#define JOY_X     2
#define JOY_Y     3
...
// Wii has no touch screen usually, but keeping struct for compatibility if needed
...
void _changeStretch(StretchOption newStretch){
    if (newStretch == PixelPerfect) {
        ...
        // Wii resolution is 640x480, so max integer scale is 3x (384x384)
        screenWidth = PicoScreenWidth * 3;
        screenHeight = PicoScreenHeight * 3;
    }
    else if (newStretch == FourByThreeVertPerfect) {
        // 4:3 is native on Wii usually
        screenWidth = PicoScreenWidth * 3;
        screenHeight = PicoScreenHeight * 3;
    }
...
Host::Host(int windowWidth, int windowHeight)  {
    ...
    // On Wii, sd:/ is usually mapped to root / or accessible
    // We'll use relative paths or sd:/ prefix if libogc supports it.
    // libogc uses sd:/ for SD card.

    // Ensure directories exist
    int res = stat("sd:/apps", &st);
    if (res == -1) mkdir("sd:/apps", 0777);

    res = stat("sd:/apps/fake08", &st);
    if (res == -1) mkdir("sd:/apps/fake08", 0777);

    res = stat("sd:/apps/fake08/cdata", &st);
    if (res == -1) mkdir("sd:/apps/fake08/cdata", 0777);

    _logFilePrefix = "sd:/apps/fake08/";
...
void Host::oneTimeSetup(Audio* audio){
	...
	window = SDL_CreateWindow(nullptr, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIN_WIDTH, WIN_HEIGHT, 0);
	if (!window)
    {
        quit = 1;
        return;
    ...
    for (int i = 0; i < SDL_NumJoysticks(); i++) {
		if (SDL_JoystickOpen(i) == NULL) {
			...
			// Don't quit if joystick fails, maybe keyboard works (usb keyboard)
		}
    }
...
InputState_t Host::scanInput(){
    ...
    while (SDL_PollEvent(&event)) {
        switch (event.type) {
            ...
            case SDL_FINGERDOWN:
                // Wii doesn't support touch usually, but logic remains
